...
# Run scans on each server and build summary report using ssllabs_client.py's methods
def process(server_list_file, check_progress_interval_secs=10,
        summary_csv=SUMMARY_CSV, summary_html=SUMMARY_HTML):
    ret = 0
    # Gather servers from input file
    ...
    # Get owner list if owner is listed next to host
    owners = []
    for i in content:
    	owner = i.strip()
